...
// --- Lid alignment rail measurements ---
lid_rail_depth = switch_spacing + 2;
lid_rail_width = 3;
lid_rail_height = total_height + 2;
// --- Lid snap-on rail measurements ---
lid_snap_rim = 0.2;
lid_snap_rim_height = -2; // Friction side tabs that are shorter than bottom case
//lid_snap_rim_height = 2; // Side tabs with bumps that extend beyond bottom of case
lid_snap_height = bottom_wall_h*2+cavity_depth*2+top_plate_h + lid_snap_rim_height;
lid_snap_width = 12;
lid_snap_depth = 3;
...
module lid_guide(dir) {
    difference() {
        cube([lid_rail_width, lid_rail_depth, lid_rail_height]);
        translate([0, lid_rail_depth/2, lid_rail_height])
            rotate([0, 90, 0])
                cylinder(r=lid_rail_depth/6, h=lid_rail_width);
    }
}
module lid_snap(dir) {
    cube([lid_snap_width, lid_snap_depth, lid_snap_height]);

    
    difference() {
        translate([0, dir < 0 ? lid_snap_rim*dir : lid_snap_depth, lid_snap_height-lid_snap_rim_height])
            cube([lid_snap_width, lid_snap_rim, lid_snap_rim_height/2]);
    }    
}
module lid_shell() {
    difference() {
    ...
    // end bars
    rail_y = (case_depth / 2) - (lid_rail_depth/2);
    
    translate([lid_rail_width*-1, rail_y, 0])
        lid_guide(1);
    translate([case_width, rail_y, 0])
        lid_guide(-1);
    
    // Side bars
    translate([case_corner_r*3, lid_snap_depth*-1, 0])
        lid_snap(1);
    translate([case_width - (case_corner_r *3 + lid_snap_width), lid_snap_depth*-1, 0])
        lid_snap(1);
    translate([case_corner_r *3, case_depth, 0])
        lid_snap(-1);
    translate([case_width - (case_corner_r *3 + lid_snap_width), case_depth, 0])
        lid_snap(-1);
}
